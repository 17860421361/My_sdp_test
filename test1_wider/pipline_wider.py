"""
Widar/Wider 完整 pipeline 测试脚本。

注意：目录名是 test_wider，但源码中的数据集名称必须是 "widar"。

源码中 widar 的关键要求：
- dataset_name 必须是 "widar"，这样 readers.load_data() 才会选择 BfeeReader。
- Widar 文件名格式为 user{用户}-{手势}-{位置}-{方向}-{序号}-r{接收器}.dat，
  例如 user2-1-3-4-5-r6.dat。
- BaseProcessor 对 widar 的解析规则：
  label = gesture_type，也就是文件名里的第 2 个数字。
  group = user_id，也就是文件名里的第 1 个数字。
- 因此数据划分应优先按用户分组划分，用于 cross-person evaluation。
- BfeeReader 输出单帧形状通常为 (30, n_rx*n_tx)，堆叠后是 (T, 30, A)。

model_params.json 中 widar 默认参数：
  "widar": {
    "batch": 32,
    "lr": 3e-4,
    "wd": 1e-3,
...
from configurable_processor_wider import ConfigurableProcessor
...
def _split_by_group_or_random(processed_data, labels, groups):
    ...
    # 第二次无法继续 GroupShuffleSplit，因此与 xrf55 测试脚本一样退回随机划分。
...
def main():
    print("=" * 80)
    print("Widar/Wider WSDP 完整 pipeline 测试")
    print("=" * 80)

    print(f"step 1，加载数据：{data_path}")
    print("   说明: dataset='widar' 会自动选择 BfeeReader")
    print("   说明: 文件名解析为 user_id / gesture_type / position / orientation / serial / receiver")
    print("   说明: 使用本地 SDP 源码默认解析，label=gesture_type, group=user_id")
    raw_file_count = sum(
        1
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("Widar/Wider pipeline 测试完成")
        print("=" * 60)
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Widar/Wider Loss Curve")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
