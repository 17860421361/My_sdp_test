"""ElderAL 80 组自动 pipeline 全量实验脚本。
...
这个文件不依赖 pipline_elderAL_steps.py，而是直接调用 WSDP 源码模块。
"""
...
INTERPOLATE_OPTIONS = [
    ("linear64", {"method": "linear", "target_K": 64}),
    ("cubic64", {"method": "cubic", "target_K": 64}),
    ("nearest64", {"method": "nearest", "target_K": 64}),
    ("decimate64", {"method": "decimate", "target_K": 64}),
]
...
def build_pipeline_combinations() -> list[dict]:
    """自动生成 ElderAL 的 80 组 pipeline 组合。"""
    combinations = []
    ...
    for combo_index, (denoise, outliers, normalize, interpolate) in enumerate(
        ...
        combo_id = f"elderAL_{combo_index:03d}"
        combo_name = "+".join(
            [
                denoise_name,
                outliers_name,
...
def split_data(
    ...
    """第三步：调用源码划分函数；ElderAL 使用 group split。"""
...
def main() -> None:
    ...
    print(f"ElderAL 全量 pipeline 组合数: {combo_total}")
