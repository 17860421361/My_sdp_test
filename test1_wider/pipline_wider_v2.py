"""
Widar/Wider pipeline v2.

This script does not modify WSDP source code. It keeps the same reader,
preprocessing steps, model, and training settings as pipline_wider.py, but uses
a non-user condition group for the split:

- label = gesture_type
- group = position_id * 1000 + orientation_id * 100 + receiver_id

This is a sanity check for whether BaseProcessor + CSIModel can learn Widar
gestures when train/valid/test are closed-set users but held-out conditions.
"""
...
dataset_name = "widar"
processor_name = "condition_v2+baseprocessor"
pipeline_steps = {
...
class WidarConditionGroupProcessor:
    """Local processor for Widar gesture recognition with non-user groups."""
    ...
    def process(self, data_list, **kwargs):
        ...
        worker_func = partial(
            _process_single_csi_condition_group,
            dataset=dataset,
...
def _process_single_csi_condition_group(csi_data, dataset, pipeline_steps):
    ...
    _user_id, gesture_type, position_id, orientation_id, _serial_id, receiver_id = parsed
    label = gesture_type
    group = position_id * 1000 + orientation_id * 100 + receiver_id
...
def main():
    print("=" * 80)
    print("Widar/Wider WSDP pipeline v2")
    print("=" * 80)
    ...
    print("   说明: label=gesture, group=position*1000+orientation*100+receiver")
    raw_file_count = sum(
        1
    ...
    processor = WidarConditionGroupProcessor(pipeline_steps)
    ...
    print(f"非用户分组分布: {_short_distribution(all_groups)}")
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("Widar/Wider pipeline v2 测试完成")
        print("=" * 60)
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Widar/Wider v2 Loss Curve")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
