...
这个文件不依赖 pipline_widar_steps.py，而是直接调用 WSDP 源码模块。
...
def build_pipeline_combinations() -> list[dict]:
    """自动生成 Widar 的 320 组 pipeline 组合。"""
    combinations = []
...
def split_data(
    ...
    """第三步：调用源码划分函数；Widar 使用 group split。"""
