"""
Run all WSDP presets against registered models on the Widar gesture task.

The batch-test flow follows test_gait/full_test_presets_models_gait.py.
The Widar-specific processing logic follows pipline_wider_v2.py:
...
from pipline_wider_v2 import WidarConditionGroupProcessor
...
summary_path = os.path.join(result_root, "condition_v2_all_presets_models_summary.csv")
resume_from_summary = True
skip_model_names = set()
result_prefix = "condition_v2"
...
def save_loss_curve(history, output_dir):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Widar Condition Split Loss Curve")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def run_one_model(preset_name, pipeline_steps, model_name, loaders, device):
    ...
    try:
        ...
        print(f"类别数：{loaders['num_classes']}，原始手势标签：{loaders['unique_labels']}")
...
def main():
    print("=" * 80)
    print("Widar Condition Split 全预设 + 全模型批量测试")
    print("=" * 80)
    ...
    print("说明: label=gesture_type, group=position*1000+orientation*100+receiver")
    ...
    for preset_idx, preset_name in enumerate(preset_names, start=1):
        ...
        print(f"分组数量: {len(set(all_groups))}")
        ...
        print("step 4，数据划分")
        print("使用 v2 condition group: position*1000+orientation*100+receiver")
