...
# 当前脚本位于 test_wider 目录下，项目根目录是它的上一级。
...
def _process_single_csi_configurable(csi_data, dataset, pipeline_steps):
    ...
    # 从本地 SDP 源码解析标签和分组。widar 当前为 label=gesture_type, group=user_id。
...
if __name__ == "__main__":
    # Widar 数据集默认路径；目录名是 test_wider，但 WSDP 内部数据集名称是 widar。
    input_path = os.path.join(PROJECT_ROOT, "data", "widar_common3")
    dataset_name = "widar"
    ...
    print("处理完成:")
    print("  widar 语义: label=gesture_type, group=user_id")
...
"""
三个用户，三类手势，五个位置，五个方向，每个位置/方向重复五次，六个接收器
原始样本6750
"""
