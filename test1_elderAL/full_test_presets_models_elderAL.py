"""
Run all WSDP presets against all registered models on the ElderAL action task.

The batch-test flow follows test_gait/full_test_presets_models_gait.py.
The ElderAL-specific processing logic follows pipline_elderAL.py and the local
SDP source code:
...
# ======================= 配置区 ========================
data_path = os.path.join(PROJECT_ROOT, "data", "elderAL")
...
summary_path = os.path.join(result_root, "source_action_position_all_presets_models_summary.csv")
resume_from_summary = True
skip_model_names = set()
result_prefix = "source_action_position"
...
def save_loss_curve(history, output_dir):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("ElderAL Action Loss Curve")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def run_one_model(preset_name, pipeline_steps, model_name, loaders, device):
    ...
    try:
        ...
        print(f"类别数：{loaders['num_classes']}，原始动作标签：{loaders['unique_labels']}")
...
def main():
    print("=" * 80)
    print("ElderAL Action 全预设 + 全模型批量测试")
    print("=" * 80)
    ...
    print("说明: label=action_id, group=position_id")
    ...
    for preset_idx, preset_name in enumerate(preset_names, start=1):
        ...
        print("step 4，数据划分")
        print("使用本地 SDP 源码 elderAL 语义: label=action_id, group=position_id")
