"""
Widar/Wider position-split pipeline.

This script is based on pipline_wider_v2.py. It keeps the same reader,
preprocessing steps, model, and training settings, but uses position only for
the train/valid/test split:

- label = gesture_type
- group = position_id

This checks whether BaseProcessor + CSIModel can recognize gestures at
held-out positions while users, orientations, receivers, and repetitions may
...
# Force this script to use CUDA device 0 when CUDA is available.
...
dataset_name = "widar"
processor_name = "position_split+baseprocessor"
pipeline_steps = {
...
class WidarPositionGroupProcessor:
    """Local processor for Widar gesture recognition with position groups."""
    ...
    def process(self, data_list, **kwargs):
        ...
        worker_func = partial(
            _process_single_csi_position_group,
            dataset=dataset,
...
def _process_single_csi_position_group(csi_data, dataset, pipeline_steps):
    ...
    _user_id, gesture_type, position_id, _orientation_id, _serial_id, _receiver_id = parsed
    label = gesture_type
    group = position_id
...
def main():
    print("=" * 80)
    print("Widar/Wider WSDP position-split pipeline")
    print("=" * 80)

    print(f"step 1，加载数据：{data_path}")
    print("   说明: dataset='widar' 会自动选择 BfeeReader")
    print("   说明: 文件名解析为 user_id / gesture / position / orientation / serial / receiver")
    print("   说明: label=gesture, group=position")
    raw_file_count = sum(
        1
    ...
    processor = WidarPositionGroupProcessor(pipeline_steps)
    ...
    print(f"位置分组分布: {_distribution(all_groups)}")
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("Widar/Wider position-split pipeline 测试完成")
        print("=" * 60)
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Widar/Wider Position Split Loss Curve")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
