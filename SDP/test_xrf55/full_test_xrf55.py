"""XRF55 80 组自动 pipeline 全量实验脚本。

固定模型：csitime

组合空间：
denoise(5) -> outliers(2) -> normalize(2) -> interpolate(4)
= 80 组 pipeline。

这个文件不依赖 pipline_xrf55_steps.py，而是直接调用 WSDP 源码模块。

XRF55 当前实验约定：
- 只读取前三个用户的数据；
- 标签是 action_id；
- 分组依据是 repetition_id；
- 划分固定为 repetition 1-12 train，13-16 valid，17-20 test；
- z-score / min-max 会在源码 _create_data_split() 中使用训练集统计量归一化。
"""
...
def build_pipeline_combinations() -> list[dict]:
    """自动生成 XRF55 的 80 组 pipeline 组合。"""
    combinations = []

    for combo_index, (denoise, outliers, normalize, interpolate) in enumerate(
        product(
            DENOISE_OPTIONS,
            OUTLIER_OPTIONS,
            NORMALIZE_OPTIONS,
        ...
        combo_id = f"xrf55_{combo_index:03d}"
        combo_name = "+".join(
            [
                denoise_name,
                outliers_name,
...
def load_raw_data():
    ...
    files = [
        ...
        and "truth" not in file_path.name
        and is_first_three_user_file(file_path)
    ]
    ...
    if skipped > 0:
        print(f"[Info] skipped {skipped} file(s) (format mismatch for {DATASET_NAME} reader)")
...
def split_data(
    ...
    """第三步：调用源码划分函数；XRF55 使用固定 repetition split。"""
...
def main() -> None:
    ...
    print(f"XRF55 全量 pipeline 组合数: {combo_total}")
    print(f"固定模型: {MODEL_NAME}")
    print(f"结果目录: {RESULT_DIR}")
    print(f"summary: {SUMMARY_PATH}")
    print(f"已完成组合数: {len(done_records)}")

    # step 1: 原始数据只读取一次，后续 80 个 pipeline 复用这份 csi_data_list。
    set_seed(SEED)
