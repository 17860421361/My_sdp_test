...
# 当前脚本位于 test_elderAL 目录下，项目根目录是它的上一级。
...
class ConfigurableProcessor:
    ...
    def process(self,data_list,**kwargs):
        """ 多余的参数以 key=value 形式传入的参数，全部打包成一个字典。"""
...
def _process_single_csi_configurable(csi_data, dataset, pipeline_steps):
    ...
    # 从本地 SDP 源码解析标签和分组。elderAL 当前为 label=action_id, group=position_id。
...
if __name__ == '__main__':
    # ElderAL 数据集默认路径和 WSDP 内部识别的数据集名称。
    ...
    print(f"处理完成:")
    print("  elderAL 语义: label=action_id, group=position_id")
...
"""
三个用户，六个动作，九个位置，每个位置重复15次，原始样本2425
"""
